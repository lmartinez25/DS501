...
####CHANGE TO GITHUB?--------
df = read.csv("C:/Users/asales/OneDrive - Worcester Polytechnic Institute (wpi.edu)/Desktop/Liliana/LilianaDS501/case3_data_clean.csv")
...
introduction = "In this case study, we use census data from 2023 in Massachusetts (MA) to overview the demographic population and determine which demographic factors play an important role in determining employment status."
...
"While logistic regression model probabilities of an outcome, decision trees output the decision (what category is predicted it will be) but both can provide the opposite as well. Both can also work with input variables that are categorical and/or continuous. In our case, we only use categorical variables.",br(),br(),

strong("How they work"),br(),

em("Logistic Regression:"),br(),
"The model sets the log odds of the outcomes equal to a linear function of the predictors. The coefficients on these predictors are calculated to best fit the data and show the effect of each predictor on the log odds of the outcome which can be converted to the probability of the outcome for the given input. The model makes classifications by setting a threshold on the probability. Found in the 'models' tabs, the summary of the logistic regression model shows us the relative and direction of impact of each predictor on the outcome.",br(),br(),
...
"-Does not handle missing values or discontinuous relationships well. An example of a discontinuous relationship could be education. If having higher level degrees makes employment far more likely than lower degrees, logistic regression will struggle to capture this since it assumes linearity.",br(), 
